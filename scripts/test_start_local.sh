...
cleanup_and_exit_failure() {
  ...
  if [ -d "./startlocal" ]; then # Check if the work directory exists
    curl "${SCRIPT_URL}" | sh -s -- clean || log_info "Cleanup command ('start-local.sh clean') also had issues."
  else
...
if [[ "${SCENARIO_TO_RUN}" == "default-run" ]]; then
  ...
  while [ $elapsed -lt $timeout_seconds ]; do
    ...
    if is_port_open "${HOST_TO_CHECK}" "${PORT_TO_CHECK}"; then
      ...
      http_code=$(curl -q --fail --connect-timeout 15 --retry 1 --retry-delay 5 \
                   -s -w "%{http_code}" \
  ...
  while [ $elapsed -lt $timeout_seconds ]; do
    ...
    if is_port_open "${HOST_TO_CHECK}" "${PORT_TO_CHECK}"; then
      ...
      health_http_code=$(curl -q --fail --connect-timeout 10 --retry 2 --retry-delay 3 \
                             -s -w "%{http_code}" \
      ...
      if [ $health_curl_exit_code -eq 0 ] && [[ "$health_http_code" == "200" ]]; then
        ...
        if jq -e '.status == "green"' "${health_body_file}" > /dev/null; then
          ...
          nodes_http_code=$(curl -q --fail --connect-timeout 10 --retry 2 --retry-delay 3 \
                                 -s -w "%{http_code}" \
                                 -ku "admin:${CUSTOM_PASSWORD}" \

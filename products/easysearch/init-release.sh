...
IFS='.' read -r major minor revision <<< "$RELEASE_VERSION"
...
if ! grep -q "public static final Version $JVER " "$FVER"; then
  ...
  # 7. 构建新的版本定义行
  RELEASE_VERSION_line="    public static final Version $JVER = new Version($id, org.apache.lucene.util.Version.LUCENE_$LUCENE_VERSION);"

  # 8. 找到已发布的版本定义，并在其下一行插入快照版本定义
  PUBLISH_JVER=$(echo "$BASE_VERSION" | tr '.' '_' | awk '{print "V_"$0}')
  sed -i "/public static final Version $PUBLISH_JVER /a\\$RELEASE_VERSION_line" "$FVER"
fi

...
JAR_FILE=$(find /root/.m2/repository/com/infinilabs/tongsuo-openjdk/ -name "tongsuo-openjdk-*-${PLATFORM_NAME}-dynamic.jar" 2>/dev/null | head -1)
if [ -n "$JAR_FILE" ] && [ -f "$JAR_FILE" ]; then
    ...
    echo "  ⚠️  JAR file not found! (Looking for: *-${PLATFORM_NAME}-dynamic.jar)"
fi
echo ""
